...
import { getGeminiAccessToken } from './google-auth'
...
export async function callGeminiImage(params: {
  ...
  if (!imagePart?.inlineData) {
    ...
    throw new Error(`Gemini returned no image. Response: ${textParts.slice(0, 200)}`)
  }
  ...
  const { base64, mimeType, originalKB, compressedKB } =
    await compressToWebP(imagePart.inlineData.data, imagePart.inlineData.mimeType || 'image/jpeg', type)
