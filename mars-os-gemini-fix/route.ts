...
// ── Gemini helpers (service account Bearer token — works on Vercel) ───────────
...
async function callGemini(prompt: string): Promise<any> {
  return withRetry(async () => {
    const data = await geminiPost(GEMINI_MODEL, {
      contents: [{ parts: [{ text: prompt }] }],
    })
    const usage = data.usageMetadata
    if (usage) addTokens((usage.promptTokenCount ?? 0) + (usage.candidatesTokenCount ?? 0))
...
async function callGeminiWithGrounding(prompt: string): Promise<{ data: any; grounding: GroundingMeta }> {
  ...
  // Pass 1: grounding search via raw fetch
  const pass1 = await withRetry(() => geminiPost(GEMINI_MODEL, {
    contents: [{ parts: [{ text: researchPart }] }],
    tools: [{ googleSearch: {} }],
  }))

  const usage1 = pass1.usageMetadata
  if (usage1) addTokens((usage1.promptTokenCount ?? 0) + (usage1.candidatesTokenCount ?? 0))

  const candidate = pass1.candidates?.[0]
  const researchText = (candidate?.content?.parts ?? []).map((p: any) => p.text ?? '').join('')
  ...
  const data = await withRetry(async () => {
    const pass2 = await geminiPost(GEMINI_MODEL, {
      contents: [{ parts: [{ text: formatPrompt }] }],
      generationConfig: { responseMimeType: 'application/json', temperature: 0.3, maxOutputTokens: 8192 },
    })
    const usage2 = pass2.usageMetadata
    if (usage2) addTokens((usage2.promptTokenCount ?? 0) + (usage2.candidatesTokenCount ?? 0))
    return parseGeminiJSON(pass2.candidates?.[0]?.content?.parts?.[0]?.text ?? '')
  })
