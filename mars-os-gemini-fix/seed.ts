...
async function main() {
  ...
  const users = await Promise.all([
    prisma.user.upsert({
      ...
      update: { password: await bcrypt.hash("Convertcakeseo01", 12) },
      create: { name: "adminseo", email: "adminseo@marsosapp.com", password: await bcrypt.hash("Convertcakeseo01", 12), role: "ADMIN", organizationId: org.id, status: "ACTIVE" },
    }),
    prisma.user.upsert({
      where: { email: "userseo@marsosapp.com" },
      update: { password: await bcrypt.hash("Convertcakeseo01", 12) },
      create: { name: "userseo", email: "userseo@marsosapp.com", password: await bcrypt.hash("Convertcakeseo01", 12), role: "WRITER", organizationId: org.id, status: "ACTIVE" },
    }),
    prisma.user.upsert({
      where: { email: "manager@marsosapp.com" },
      update: {},
      create: { name: "SEO Manager", email: "manager@marsosapp.com", password: await bcrypt.hash("Convertcakeseo01", 12), role: "SEO_MANAGER", organizationId: org.id, status: "ACTIVE" },
    }),
    prisma.user.upsert({
      where: { email: "writer@marsosapp.com" },
      update: {},
      create: { name: "Content Writer", email: "writer@marsosapp.com", password: await bcrypt.hash("Convertcakeseo01", 12), role: "WRITER", organizationId: org.id, status: "ACTIVE" },
    }),
    prisma.user.upsert({
      where: { email: "reviewer@marsosapp.com" },
      update: {},
      create: { name: "SEO Reviewer", email: "reviewer@marsosapp.com", password: await bcrypt.hash("Convertcakeseo01", 12), role: "REVIEWER", organizationId: org.id, status: "ACTIVE" },
    }),
    prisma.user.upsert({
      where: { email: "publisher@marsosapp.com" },
      update: {},
      create: { name: "Content Publisher", email: "publisher@marsosapp.com", password: await bcrypt.hash("Convertcakeseo01", 12), role: "PUBLISHER", organizationId: org.id, status: "ACTIVE" },
    }),
  ]);
