...
function toRecord(url: string, status: number, ex: ExtractedPage, rendered: boolean, redirectedTo: string | null): PageRecord {
  ...
  return {
    ...
    h3: ex.h3.slice(0, 20),
    questionHeadings: ex.questionHeadings,
    citationLinks: ex.citationLinks,
    authorName: ex.authorName,
    hasSummaryBlock: ex.hasSummaryBlock,
    leadWordCount: ex.leadWordCount,
    answersInLead: ex.answersInLead,
    images: ex.images,
    imagesWithAlt: ex.imagesWithAlt,
    pageType: 'other',
