...
export function buildCacheKey(input: SkillInput, namespace = ''): string {
  const parts = [
    ...
    // geo/language resource ต้องอยู่ใน key — ไม่งั้นผลระดับเขตกับระดับประเทศชนกัน
    (input.google_ads_geo_target_resources || []).join(','),
    input.google_ads_language_resource || '',
...
export function readKeywordPlannerCache(key: string, opts?: { allowStale?: boolean }): CacheEntry | null {
  try {
    ensureCacheDir();
    ...
    // allowStale: ไพ่ใบสุดท้ายตอน Google Ads ล่ม — ของเก่าเกิน TTL ยังดีกว่าไม่มีเลย
    if (age > TTL_MS && !opts?.allowStale) return null; // expired
    return entry;
  } catch {
    return null;
  }
