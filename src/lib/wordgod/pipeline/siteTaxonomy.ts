...
/** Local minimal shape for a deep-crawl result — the full `crawlSiteDeep`
 *  service (and the `/api/crawl-site-deep` route that produces this) was not
 *  ported in this pass; `derivePillarsFromCrawl` below is kept for structural
 *  parity but is unused until that service is ported. */
...
function derivePillars(
  ...
  if (categoryList && categoryList.length > 0) {
    ...
    groups = Array.from(bySeg.values());
  }
