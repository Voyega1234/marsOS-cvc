...
function duplicateValues(values: string[]): string[] {
  ...
  return Array.from(count.entries()).filter(([, n]) => n > 1).map(([value]) => value);
}
...
export function buildContentPlan(
  ...
  for (const list of Array.from(itemsByPillar.values())) {
    list.sort((a, b) => {
    ...
    for (const item of list) {
      ...
      item.internalLinks = Array.from(new Set(links.filter(Boolean))).slice(0, 3);
    }
  }

  const derivedPillars: PlanPillarInput[] = explicitPillars.length > 0
    ? explicitPillars
    : Array.from(itemsByPillar.keys()).map(name => ({ name }));
  ...
  for (const [name, items] of Array.from(itemsByPillar)) {
  ...
  for (let monthIndex = 0; monthIndex < config.months; monthIndex++) {
    ...
    for (const [pillarName, pool] of Array.from(orderedPools)) {
  ...
  const pillars: PillarPlan[] = Array.from(itemsByPillar.entries()).map(([name, items]) => ({
    name,
